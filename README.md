...
| 多个专家意见可能冲突，角色也不应每轮轮流发言 | 固定多 Agent 流程会增加延迟、成本与重复结论。 | 编排器只判断当前谁有新增专业价值，输出 `act / wait / hold`；暂无新增动作时保留会谈继续权。 |
...
| 3 | [工具与运行](docs/TOOLS_AND_RUNTIME.md) | 工具包、授权、证据、运行治理与角色能力扩展标准。 |
| 4 | [角色设计](docs/ROLES.md) | 五类职业角色的主成果、边界与扩展标准。 |
| 5 | [动态会谈协议](docs/MEETING_PROTOCOL.md) | 一次会谈如何从新事件走向角色选择、交付、等待或停止。 |
| 6 | [记忆系统](docs/MEMORY_SYSTEM.md) | 原始档案、工作记忆、上下文压缩和用户更正治理。 |
| 7 | [安全与边界](docs/SAFETY_AND_COMPLIANCE.md) | 语义判断与运行规则如何分界，如何保护角色与工具边界。 |
| 8 | [评估方法](docs/EVALUATION.md) | 如何按用户价值、专业能力、证据质量与可靠性验收。 |
...
```mermaid
flowchart TB
    U[用户表达 / 补充 / 更正] --> E[来源化会谈事件]
    R[已批准角色报告] --> E
    X[Research / 工具证据] --> E

    E --> V[会谈版本协调器\n提交完整快照、幂等与恢复]
    V --> M[自然语言工作记忆\n最新事实、更正、证据、未决问题]
    M --> O[自治编排器\n管理角色租约：act / wait / hold]
    O --> A[当前角色的自治入口\nEleanor 已实现的参考路径]

    subgraph ROLE[角色本轮自治闭环]
        A --> D[组合语义决策\n理解意图、设定阶段目标、形成专业正文]
        D --> T{工具结果是否会\n改变本轮行动？}
        T -- 否 --> Q[安全审计\n仅隔离危险行为]
        T -- 是 --> C[工具价值合同\n说明为何工具会改变行动]
        C --> G[统一授权 + Manifest 语义落地]
        G --> B[WPMT 工具桥接\n编译参数、执行、审计]
        B --> X2[Evidence Envelope\n事实、来源、时点、限制]
        X2 --> I[证据解释决策\n将证据回写到方案]
        I --> Q
        Q --> P[无损人格化交付\n只润色，不重判、不摘要]
    end
